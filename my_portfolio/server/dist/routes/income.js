...
router.get("/", authMiddleware_1.default, async (req, res) => {
    try {
        const incomes = await Income_1.default.find({ userId: req.userId })
            .sort({ date: -1 });
        res.json(incomes);
    }
    catch (error) {
...
router.put("/:id", authMiddleware_1.default, async (req, res) => {
    try {
        const updated = await Income_1.default.findOneAndUpdate({
...
router.delete("/:id", authMiddleware_1.default, async (req, res) => {
    try {
        const deleted = await Income_1.default.findOneAndDelete({
            _id: req.params.id,
            userId: req.userId,
        });
        if (!deleted) {
